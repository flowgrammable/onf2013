...
Bridge::flow_mod_none(const Time& t)
{
  ...
  instructions.push_back(Instruction_apply_actions(actions));
  
  Message *msg = new Message(Flow_mod(0, 0, 0, Flow_mod::ADD, 0, -1, 
                             none_priority, 0, Port::FLOOD, 0, 
                             Flow_mod::SEND_FLOW_REM, m, instructions));
...
Bridge::flow_mod_src(const Ethernet_addr& addr, uint16_t port, const Time& t)
{
  ...
  Message *msg = new Message(Flow_mod(0, 0, 0, Flow_mod::ADD, 0, hard_timeout, 
                             src_priority, 0, Port::FLOOD, 0, 
                             Flow_mod::SEND_FLOW_REM, m, instructions));
  send(msg);
}
