...
#ifndef FLOWGRAMMABLE_PROTO_OFP_V1_3_1_H
#define FLOWGRAMMABLE_PROTO_OFP_V1_3_1_H
...
enum Error_code {
  ...
  BAD_HELLO_ELEMENT,         ///< Invalid value of Hello_element_type
  BAD_HELLO_ELEMENT_LENGTH,  ///< The Hello_element length is invalid
  BAD_HELLO_ELEMENT_INIT,    ///< An uninitialized Hello_element was validated
  ...
  AVAILABLE_VERSION_BITMAP,     ///< Not enough bytes for a Version_bitmap
  /// Not enough bytes for a Hello_element_version_bitmap
  AVAILABLE_HELLO_ELEMENT_VERSION_BITMAP,
  /// Not enough bytes for a Hello_element_header
  AVAILABLE_HELLO_ELEMENT_HEADER,
  /// Not enough bytes for a Hello_element_payload
  AVAILABLE_HELLO_ELEMENT_PAYLOAD,
  ...
  /// Extra bytes when processing a sequence of type Hello_element
  EXCESS_HELLO_ELEMENT,
...
enum Hello_element_type : uint16_t
{
  VERSIONBITMAP = 1
};

Error_condition is_valid(Hello_element_type het);

std::string to_string(Hello_element_type);

template<Hello_element_type K>
  using Hello_element_payload_base = Basic_payload_base<Hello_element_type, K>;
...
{
  Version_bitmap() = default;
  explicit Version_bitmap(uint32_t b)
    : bitmap(b) { }
  
  uint32_t bitmap;
};

bool operator==(const Version_bitmap& a, const Version_bitmap& b);

bool operator!=(const Version_bitmap& a, const Version_bitmap& b);

constexpr std::size_t bytes(const Version_bitmap&);

constexpr Error_condition is_valid(const Version_bitmap&);

Error_condition to_buffer(Buffer_view& v, const Version_bitmap& vb);

Error_condition from_buffer(Buffer_view& v, Version_bitmap& vb);

std::string to_string(const Version_bitmap& vb, Formatter& f);

// -------------------------------------------------------------------------- //
// Hello element version bitmap

struct Hello_element_version_bitmap
  : Hello_element_payload_base<VERSIONBITMAP>
{
  Hello_element_version_bitmap() = default;
  explicit Hello_element_version_bitmap(const Sequence<Version_bitmap> b)
    : bitmaps(b) { }

  Sequence<Version_bitmap> bitmaps;
};

bool operator==(const Hello_element_version_bitmap& a,
                const Hello_element_version_bitmap& b);

bool operator!=(const Hello_element_version_bitmap& a,
                const Hello_element_version_bitmap& b);

std::size_t bytes(const Hello_element_version_bitmap& vb);

Error_condition is_valid(const Hello_element_version_bitmap& vb);

Error_condition to_buffer(Buffer_view&, const Hello_element_version_bitmap&);

Error_condition from_buffer(Buffer_view&, Hello_element_version_bitmap&);

std::string to_string(const Hello_element_version_bitmap&, Formatter&);

// -------------------------------------------------------------------------- //
// Hello element payload

union Hello_element_payload_data
{
  Hello_element_payload_data() { }
  ~Hello_element_payload_data() { }

  Hello_element_version_bitmap version_bitmap;
};

struct Hello_element_payload
{
  using Data = Hello_element_payload_data;

  Hello_element_payload() : init(false) { }
...
void construct(Hello_element_payload&, Hello_element_type);
void construct(Hello_element_payload& p, const Hello_element_payload&& x,
               Hello_element_type t);
void construct(Hello_element_payload&, const Hello_element_payload&,
               Hello_element_type);

Hello_element_payload& assign(Hello_element_payload&,
                             const Hello_element_payload&&, Hello_element_type);
Hello_element_payload& assign(Hello_element_payload&,
                              const Hello_element_payload&, Hello_element_type);

void destroy(Hello_element_payload&, Hello_element_type);

bool equal(const Hello_element_payload&, const Hello_element_payload&,
           Hello_element_type, Hello_element_type);

std::size_t bytes(const Hello_element_payload&, Hello_element_type);
Error_condition is_valid(const Hello_element_payload&,
                         const Hello_element_type);

Error_condition to_buffer(Buffer_view&, const Hello_element_payload&,
                          const Hello_element_type);

Error_condition from_buffer(Buffer_view&, Hello_element_payload&,
                            Hello_element_type);

std::string to_string(const Hello_element_payload&, Formatter&,
                      Hello_element_type);
...
struct Hello_element_header
{
  Hello_element_header() = default;
  Hello_element_header(Hello_element_type t, uint16_t l) : type(t), length(l) {}

  template<typename T>
    Hello_element_header(const T& value)
      : type(T::kind), length(4 + bytes(value)) { }

  Hello_element_type type;
  uint16_t length;
};

bool operator==(const Hello_element_header& a, const Hello_element_header& b);

bool operator!=(const Hello_element_header& a, const Hello_element_header& b);

constexpr std::size_t bytes(const Hello_element_header&);

Error_condition is_valid(const Hello_element_header& h);

Error_condition to_buffer(Buffer_view&, const Hello_element_header&);

Error_condition from_buffer(Buffer_view&, Hello_element_header&);

std::string to_string(const Hello_element_header&, Formatter&);

// -------------------------------------------------------------------------- //
// Hello element

struct Hello_element
{
  using Header = Hello_element_header;
  using Payload = Hello_element_payload;

  Hello_element() = default;
  //~Hello_element();
  template <typename T>
    Hello_element(const T& v)
      : header(v) {
      construct(payload, v);
    }
  
  Hello_element(Hello_element&&);
  Hello_element& operator=(Hello_element&&);

  Hello_element(const Hello_element&);
  Hello_element& operator=(const Hello_element&);

  Header  header;
  Payload payload;
};

bool operator==(const Hello_element& a, const Hello_element& b);

bool operator!=(const Hello_element& a, const Hello_element& b);

std::size_t bytes(const Hello_element& he);

Error_condition is_valid(const Hello_element& he);

Error_condition to_buffer(Buffer_view& v, const Hello_element& he);

Error_condition from_buffer(Buffer_view& v, Hello_element& he);

std::string to_string(const Hello_element&, Formatter&);
...
struct Hello : Payload_base<HELLO>
{
  ...
  Hello() = default;
  explicit Hello(const Sequence<Hello_element> e) : elements(e) { }

  Sequence<Hello_element> elements;
};
...
struct Group_mod : Payload_base<GROUP_MOD>
{
  ...
  enum Id_value : uint32_t {
    ID_MAX = 0xffffff00, ID_ALL = 0xfffffffc, ID_ANY = 0xffffffff
  };

  Group_mod() = default;
  Group_mod(Command c, Type t, Id_value gid,
...
/// Returns a string representation of a Group_mod::Id_value.
///
/// \relates Group_mod
std::string to_string(Group_mod::Id_value);
