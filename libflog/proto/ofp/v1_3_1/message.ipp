...
#ifndef FLOWGRAMMABLE_PROTO_OFP_V1_3_1_IPP
#define FLOWGRAMMABLE_PROTO_OFP_V1_3_1_IPP
...
inline Error_condition
is_valid(Hello_element_type het) {
  return ok(het == VERSIONBITMAP,BAD_HELLO_ELEMENT);
}
...
inline bool
operator==(const Version_bitmap& a, const Version_bitmap& b) {
...
constexpr std::size_t
bytes(const Version_bitmap&) { return 4; }

constexpr Error_condition
is_valid(const Version_bitmap&) { return SUCCESS; }

inline Error_condition
to_buffer(Buffer_view& v, const Version_bitmap& vb) {
  if (not available(v, bytes(vb)))
    return AVAILABLE_VERSION_BITMAP;
  to_buffer(v, vb.bitmap);
  return SUCCESS;
}

inline Error_condition
from_buffer(Buffer_view& v, Version_bitmap& vb) {
  if (not available(v, bytes(vb)))
    return AVAILABLE_VERSION_BITMAP;
  from_buffer(v, vb.bitmap);
  return SUCCESS;
}

inline std::string
to_string(const Version_bitmap& vb, Formatter& f) {
  std::stringstream ss;
  open_block(ss, f, "Version Bitmap");

  nvp_to_string(ss, f, "Bitmap", vb.bitmap);

  close_block(ss, f, "Version Bitmap");
  return ss.str();
}
...
inline bool
operator==(const Hello_element_version_bitmap& a,
...
inline bool
operator!=(const Hello_element_version_bitmap& a,
           const Hello_element_version_bitmap& b) { return !(a == b); }

inline std::size_t
bytes(const Hello_element_version_bitmap& vb) { return bytes(vb.bitmaps); }

inline Error_condition
is_valid(const Hello_element_version_bitmap& vb) { return is_valid(vb.bitmaps);}

// -------------------------------------------------------------------------- //
// Hello element header

inline bool
operator==(const Hello_element_header& a, const Hello_element_header& b) {
...
constexpr std::size_t
bytes(const Hello_element_header&) { return 4; }

inline Error_condition
is_valid(const Hello_element_header& h) {
  return ok(h.length >= bytes(h), BAD_HELLO_ELEMENT_LENGTH);
}
...
//inline
//Hello_element::~Hello_element() { destroy(payload, header.type); }
  
inline
Hello_element::Hello_element(Hello_element&& x)
  : header(std::move(x.header)) {
  construct(payload, std::move(x.payload), header.type);
}

inline Hello_element&
Hello_element::operator=(Hello_element&& x) {
  header = std::move(x.header);
  assign(payload, std::move(x.payload), header.type);
  return *this;
}
...
inline
Hello_element::Hello_element(const Hello_element& x)
  : header(x.header) {
  construct(payload, x.payload, header.type);
}

inline Hello_element&
Hello_element::operator=(const Hello_element& x) {
  header = x.header;
  assign(payload, x.payload, header.type);
  return *this;
}
...
inline bool
operator==(const Hello_element& a, const Hello_element& b) {
  return a.header == b.header
     and equal(a.payload, b.payload, a.header.type, b.header.type);
}

inline bool
operator!=(const Hello_element& a, const Hello_element& b) { return !(a == b); }

inline std::size_t
bytes(const Hello_element& he) {
  return bytes(he.header) + bytes(he.payload, he.header.type);
}

inline Error_condition
is_valid(const Hello_element& he) {
  if(Error_decl err = is_valid(he.header))
    return err;
  return is_valid(he.payload, he.header.type);
}
...
inline std::size_t
bytes(const Hello& h) { return bytes(h.elements); }

inline Error_condition
is_valid(const Hello& h) { return is_valid(h.elements); }
...
is_valid(const Table_mod::Config_type c) {
  return ok(c == Table_mod::DEPRECATED_MASK, BAD_TABLE_MOD_CONFIG);
}
