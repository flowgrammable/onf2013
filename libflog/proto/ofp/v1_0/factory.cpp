...
/// \brief Make a version 1.0 hello message
///
/// The param version_bitmap is not used in this version
Message*
Factory::make_hello(uint32_t version_bitmap) { return make<Hello>(gen()); }

/// \brief Make a version 1.0 hello error message
///
/// \todo This needs to take buf as an argument.
Message*
...
/// \brief Make a version 1.0 features request message
Message*
Factory::make_feature_req() { return make<Feature_req>(gen()); }

/// \brief Make a version 1.0 features response message
Message* 
Factory::make_feature_res(const uint64_t datapath_id,
  ...
  return make<Feature_res>(gen(),
                           ...
                           n_buffers,
                           n_tables,
                           Feature_res::Capability(capabilities),
                           Feature_res::Action(actions),
                           ports);
}

/// \brief Make a version 1.0 flow mod message
Message*
...
/// \brief Make a version 1.0 packet out message
Message*
...
/// \brief Make a version 1.0 echo request message
Message*
Factory::make_echo_req() {
  return make<Echo_req>(gen());
}

/// \brief Make a version 1.0 echo response message
Message*
