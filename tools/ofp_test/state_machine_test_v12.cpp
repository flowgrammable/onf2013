...
#include <libflog/proto/ofp/v1_2/factory.hpp>
#include <libflog/proto/ofp/v1_2/state.hpp>

using namespace flog;
using namespace flog::ofp;
using namespace flog::ofp::v1_2;

struct Test_agent : v1_2::Agent {
...
int switch_test() {
  ...
  FSM_config c(FSM_config::v1_2, FSM_config::a1_2, tc);
  FSM_switch fsm_sw(c, xid, ta);

  if (fsm_sw.state != FSM_switch::IDLE) {
    std::cerr << "FAIL: switch did not read IDLE state" << std::endl;
    return -1;
  }
  ...
  if (fsm_sw.state != FSM_switch::FEATURE_WAIT) {
    std::cerr << "FAIL: switch did not reach FEATURE_WAIT state"<< std::endl;
    return -1; 
  }

  // Test 2: Verify fsm_sw is in ESTABLISHED state
  v1_2::Factory f = v1_2::Message::factory(xid);
  recv(fsm_sw, Time(2), *(f.make_feature_req()));

  if (fsm_sw.state != FSM_switch::ESTABLISHED) {
    std::cerr << "FAIL: switch did not reach ESTABLISHED state" << std::endl;
    return -1;
  }
  ...
  if (fsm_sw.state != FSM_switch::IDLE) {
    std::cerr << "FAIL: fini did not set state back to IDLE" << std::endl;
    return -1;
  }
  ...
  time(fsm_sw, t);
  if (fsm_sw.state != FSM_switch::FAIL) {
    std::cerr << "FAIL: switch did not reach FAIL state after feature_req timeout " << std::endl;
    return -1;
  }
...
struct Test_app : public v1_2::Application {
  void init(const Time& t) {}
};
...
int controller_test()
{
  ...
  if (fsm_ctl.state != FSM_controller::IDLE) {
    std::cerr << "FAIL: controller did not reach IDLE state " << std::endl;
    return -1;
  }
  ...
  if (fsm_ctl.state != FSM_controller::FEATURE_WAIT) {
    std::cerr << "FAIL: controller did not reach FEATURE_WAIT state"<< std::endl;
    return -1; 
  }
  ...
  if (fsm_ctl.state != FSM_controller::ESTABLISHED) {
    std::cerr << "FAIL: controller did not reach ESTABLISHED state" << std::endl;
    return -1;
  }
  time(fsm_ctl, Time(7));
  ...
  if (fsm_ctl.state != FSM_controller::FAIL) {
      std::cout << "FAIL: controller did not reach FAIL state after echo timeout" << '\n';
      return -1;
  }
  ...
  time(fsm_ctl, t);
  if (fsm_ctl.state != FSM_controller::FAIL) {
    std::cerr << "FAIL: controller did not reach FAIL state after feature_req timeout " << std::endl;
    return -1;
  }
  return 0;
}
